...
function formatBucket(util: number | null, reset: number | null, windowSec: number, nowSec: number): string {
...
const sessionStatus = define({
  ...
  run: async () => {
    ...
    const decision = shouldSkip(rows, nowSec, { staleThresholdSec: RATE_LIMIT_STALE_THRESHOLD_SEC })
