...
export async function showHelp(
  ctx: { name: string | undefined; commandPath: string[]; env: { name: string | undefined; renderUsage: (ctx: any) => Promise<string> }; log: (msg: string) => void },
  ...
  const currentName = ctx.commandPath.length > 0
    ? ctx.commandPath.at(-1)!
    : ctx.name ?? ''
