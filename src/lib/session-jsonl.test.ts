...
describe('session-jsonl', () => {
  ...
  describe('streamSessionLines', () => {
    test('parses each JSONL line as JSON object', async () => {
      const path = await writeTempFile('basic.jsonl', [
      ...
      expect(lines[1]).toEqual({ type: 'assistant', message: { content: [{ type: 'text', text: 'hi' }] } })
    })

    test('skips empty lines', async () => {
      const path = await writeTempFile('empty-lines.jsonl', [
    ...
    test('handles file with trailing newline', async () => {
      const path = await writeTempFile('trailing.jsonl',
    ...
    test('yields lines in order', async () => {
      const path = await writeTempFile('order.jsonl', [
        '{"n":1}',
        '{"n":2}',
        '{"n":3}',
      ].join('\n'))
  ...
  describe('countLines', () => {
    test('counts lines in a JSONL file', async () => {
      const path = await writeTempFile('count.jsonl', [
        '{"type":"user"}',
        '{"type":"assistant"}',
        '{"type":"summary"}',
      ].join('\n'))
    ...
    test('skips empty lines in count', async () => {
      const path = await writeTempFile('count-empty.jsonl', [
        '{"type":"user"}',
        '',
        '{"type":"assistant"}',
        '',
